 ...
 * 
 * import com.viseo.fx.config.WebConfig; import com.viseo.fx.model.User; import
 ...
 * 
 * @Test public void testGetUserById() throws Exception { User user = new
 * User("101", "A", "abc@gmail.com");
 * when(userService.getUserById("101")).thenReturn(user);
 * mockMvc.perform(get("/user/profile/101")) .andExpect(status().isOk())
 * .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
 * .andExpect(jsonPath("$.userId", is("101")))
 * .andExpect(jsonPath("$.pricingTier", is("A"))) .andExpect(jsonPath("$.email",
 * is("abc@gmail.com"))); verify(userService, times(1)).getUserById("101");
 * verifyNoMoreInteractions(userService); }
 * 
 * @Test public void testGetUserByIdFailNotFound() throws Exception {
 * when(userService.getUserById("104")).thenReturn(null);
 * mockMvc.perform(get("/user/profile/104")) .andExpect(status().isNotFound());
 * 
 * } }
 */